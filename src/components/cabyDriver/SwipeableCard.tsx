...
const SwipeableCard: React.FC<SwipeableCardProps> = ({
  ...
  const acceptGlow = useTransform(x, [0, 150], ['inset 0 0 0 0 rgba(34,197,94,0)', 'inset 0 0 60px 0 rgba(34,197,94,0.15)']);
  const rejectGlow = useTransform(x, [-150, 0], ['inset 0 0 60px 0 rgba(239,68,68,0.15)', 'inset 0 0 0 0 rgba(239,68,68,0)']);
  ...
  if (exitDirection) {
    return (
      <motion.div
        ...
        animate={{ x: exitDirection === 'right' ? 600 : -600, opacity: 0, rotate: exitDirection === 'right' ? 25 : -25 }}
    ...
    >
      {isTop && (
        <motion.div
          ...
          style={{ boxShadow: acceptGlow }}
        />
      )}
      {isTop && (
        <motion.div
          className="absolute inset-0 rounded-[2rem] pointer-events-none z-10"
          style={{ boxShadow: rejectGlow }}
        />
      )}
      {isTop && (
        <motion.div
